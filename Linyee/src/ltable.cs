...
namespace Linyee
{
	...
	public partial class Linyee
	{
		...
		*/
		...
		internal static Node hashpow2(Table t, ly_Number n) { return gnode(t, (int)lmod(n, SizeNode(t))); }
		...
		*/
		...
		/*
		** number of ints inside a ly_Number
		*/
		public const int numints = sizeof(ly_Number) / sizeof(int);


		//static const Node dummynode_ = {
		//{{null}, LINYEE_TNIL},  /* value */
		  //{{{null}, LINYEE_TNIL, null}}  /* key */
		//};
		public static Node dummynode_ = new Node(new LinyeeTypeValue(new Value(), LINYEE_TNIL), new TKey(new Value(), LINYEE_TNIL, null));
		...
		*/
		private static int findindex (LinyeeState L, Table t, StkId key) {
		  int i;
		  ...
		  if (0 < i && i <= t.sizearray)  /* is `key' inside array part? */
			...
			do {  /* check whether `key' is somewhere in the chain */
			  ...
			  if ((LinyeeORawEqualObj(key2tval(n), key) != 0) ||
			...
			LinyeeGRunError(L, "invalid key to " + LINYEE_QL("next"));  /* key not found */
			return 0;  /* to avoid warnings */
		  }
		}


		public static int luaH_next (LinyeeState L, Table t, StkId key) {
		...
		*/
		...
		private static int computesizes (int[] nums, ref int narray) {
		  ...
		  narray = n;
		  LinyeeAssert(narray/2 <= na && na <= narray);
		  return na;
		}
		...
		private static void setarrayvector (LinyeeState L, Table t, int size) {
		  int i;
		  LinyeeMReallocVector<TValue>(L, ref t.array, t.sizearray, size/*, TValue*/);
		...
		private static void setnodevector (LinyeeState L, Table t, int size) {
		  int lsize;
		  if (size == 0) {  /* no elements to hash part? */
			...
			lsize = CeilLog2(size);
			if (lsize > MAXBITS)
			  LinyeeGRunError(L, "table overflow");
			size = TwoTo(lsize);
			Node[] nodes = LinyeeMNewVector<Node>(L, size);
			t.node = nodes;
			for (i=0; i<size; i++) {
			  Node n = gnode(t, i);
			  gnext_set(n, null);
		...
		private static void resize (LinyeeState L, Table t, int nasize, int nhsize) {
		  int i;
		  ...
		  if (nasize < oldasize) {  /* array part must shrink? */
			...
			/* shrink array */
			LinyeeMReallocVector<TValue>(L, ref t.array, oldasize, nasize/*, TValue*/);
		  }
		  ...
		  if (nold[0] != dummynode)
			LinyeeMFreeArray(L, nold);  /* free old array */
		}


		public static void luaH_resizearray (LinyeeState L, Table t, int nasize) {
		  int nsize = (t.node[0] == dummynode) ? 0 : SizeNode(t);
		  resize(L, t, nasize, nsize);
		}


		private static void rehash (LinyeeState L, Table t, TValue ek) {
		  int nasize, na;
		...
		*/
		...
		public static Table luaH_new (LinyeeState L, int narray, int nhash) {
		  Table t = LinyeeMNew<Table>(L);
		  LinyeeCLink(L, obj2gco(t), LINYEE_TTABLE);
		  t.metatable = null;
		  t.flags = CastByte(~0);
		...
		public static void luaH_free (LinyeeState L, Table t) {
		  if (t.node[0] != dummynode)
			LinyeeMFreeArray(L, t.node);
		  LinyeeMFreeArray(L, t.array);
		  LinyeeMFree(L, t);
		}
		...
		*/
		private static TValue newkey (LinyeeState L, Table t, TValue key) {
		  ...
		  if (!TTIsNil(gval(mp)) || mp == dummynode) {
			...
			LinyeeAssert(n != dummynode);
			othern = mainposition(t, key2tval(mp));
			if (othern != mp) {  /* is colliding node out of its main position? */
			  /* yes; move colliding node into free position */
			  while (gnext(othern) != mp) othern = gnext(othern);  /* find previous */
			  gnext_set(othern, n);  /* redo the chain with `n' in place of `mp' */
			  n.i_val = new LinyeeTypeValue(mp.i_val);	/* copy colliding node into free pos. (mp.next also goes) */
		  ...
		  LinyeeCBarrierT(L, t, key);
		  LinyeeAssert(TTIsNil(gval(mp)));
		  return gval(mp);
		}
		...
		*/
		...
		public static TValue luaH_set (LinyeeState L, Table t, TValue key) {
		  TValue p = luaH_get(t, key);
		  t.flags = 0;
		  if (p != LinyeeONilObject)
			return (TValue)p;
		  else {
			if (TTIsNil(key)) LinyeeGRunError(L, "table index is nil");
			else if (TTIsNumber(key) && luai_numisnan(NValue(key)))
			  LinyeeGRunError(L, "table index is NaN");
			return newkey(L, t, key);
		  }
		}


		public static TValue luaH_setnum (LinyeeState L, Table t, int key) {
		  TValue p = luaH_getnum(t, key);
		  if (p != LinyeeONilObject)
			return (TValue)p;
		  else {
			TValue k = new LinyeeTypeValue();
			SetNValue(k, CastNum(key));
			return newkey(L, t, k);
		  }
		}

		public static TValue luaH_setstr (LinyeeState L, Table t, TString key) {
