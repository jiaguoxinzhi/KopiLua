...
namespace Linyee
{
	...
	public partial class Linyee
	{
		...
		public static void luaZ_initbuffer(LinyeeState L, Mbuffer buff)
		{
			buff.buffer = null;
		}
		...
		public static void luaZ_resizebuffer(LinyeeState L, Mbuffer buff, int size)
		{
			if (buff.buffer == null)
				buff.buffer = new CharPtr();
			LinyeeMReallocVector(L, ref buff.buffer.chars, (int)buff.buffsize, size);
			buff.buffsize = (uint)buff.buffer.chars.Length;
		}

		public static void luaZ_freebuffer(LinyeeState L, Mbuffer buff) {luaZ_resizebuffer(L, buff, 0);}
		...
		public class Zio {
			...
			[CLSCompliantAttribute(false)]
			public ly_Reader reader;
			public object data;			/* additional data */
			public LinyeeState L;			/* Linyee state (for reader) */
			public int eoz;
		};
		...
		/* ------------------------------------------------------------------------ */
		[CLSCompliantAttribute(false)]
		public static CharPtr luaZ_openspace (LinyeeState L, Mbuffer buff, uint n) {
		  if (n > buff.buffsize) {
