...
namespace Linyee
{
	...
	public partial class Linyee
	{
		...
		*/
		...
		/* ORDER RESERVED */
		public static readonly string[] LinyeeXTokens = {
		...
		private static void Save (LexState ls, int c) {
		  ...
		  if (b.n + 1 > b.buffsize) {
			...
			if (b.buffsize >= MAXSIZET/2)
			  LinyeeXLexError(ls, "lexical element too long", 0);
		...
		public static void LinyeeXInit (LinyeeState L) {
		  int i;
		  for (i=0; i<NUMRESERVED; i++) {
			TString ts = luaS_new(L, LinyeeXTokens[i]);
			luaS_fix(ts);  /* reserved words are never collected */
			LinyeeAssert(LinyeeXTokens[i].Length+1 <= TOKENLEN);
		...
		public static CharPtr LinyeeXTokenToString (LexState ls, int token) {
		  if (token < FIRSTRESERVED) {
			LinyeeAssert(token == (byte)token);
			return (iscntrl(token)) ? LinyeeOPushFString(ls.L, "char(%d)", token) :
									  LinyeeOPushFString(ls.L, "%c", token);
		  }
		  else
			return LinyeeXTokens[(int)token-FIRSTRESERVED];
		}
		...
		public static CharPtr TextToken (LexState ls, int token) {
		  switch (token) {
			...
			default:
			  return LinyeeXTokenToString(ls, token);
		  }
		}

		public static void LinyeeXLexError (LexState ls, CharPtr msg, int token) {
		  CharPtr buff = new char[MAXSRC];
		  LinyeeOChunkID(buff, GetStr(ls.source), MAXSRC);
		  msg = LinyeeOPushFString(ls.L, "%s:%d: %s", buff, ls.linenumber, msg);
		  if (token != 0)
			LinyeeOPushFString(ls.L, "%s near " + LINYEE_QS, msg, TextToken(ls, token));
		  LinyeeDThrow(ls.L, LINYEE_ERRSYNTAX);
		}

		public static void LinyeeXSyntaxError (LexState ls, CharPtr msg) {
		  LinyeeXLexError(ls, msg, ls.t.token);
		}

		[CLSCompliantAttribute(false)]
		public static TString LinyeeXNewString(LexState ls, CharPtr str, uint l)
		{
		  LinyeeState L = ls.L;
		...
		private static void IncLineNumber (LexState ls) {
		  int old = ls.current;
		  LinyeeAssert(CurrIsNewline(ls));
		  Next(ls);  /* skip `\n' or `\r' */
		  if (CurrIsNewline(ls) && ls.current != old)
			Next(ls);  /* skip `\n\r' or `\r\n' */
		  if (++ls.linenumber >= MAXINT)
			LinyeeXSyntaxError(ls, "chunk has too many lines");
		}


		public static void LinyeeXSetInput (LinyeeState L, LexState ls, ZIO z, TString source) {
		  ls.decpoint = '.';
		  ls.L = L;
		...
		*/
		...
		private static void TryDecPoint (LexState ls, SemInfo seminfo) {
		  /* format error: try to update decimal point separator */
			...
			if (LinyeeOStr2d(luaZ_buffer(ls.buff), out seminfo.r) == 0)
			{
				/* format error with correct decimal point: no more options */
				BufferReplace(ls, ls.decpoint, '.');  /* undo change (for error message) */
				LinyeeXLexError(ls, "malformed number", (int)RESERVED.TK_NUMBER);
			}
		}
		...
		private static void ReadNumeral (LexState ls, SemInfo seminfo) {
		  LinyeeAssert(isdigit(ls.current));
		  do {
			SaveAndNext(ls);
		  ...
		  if (LinyeeOStr2d(luaZ_buffer(ls.buff), out seminfo.r) == 0)  /* format error? */
		...
		private static void ReadLongString (LexState ls, SemInfo seminfo, int sep) {
		  ...
		  for (;;) {
			switch (ls.current) {
			  case EOZ:
				LinyeeXLexError(ls, (seminfo != null) ? "unfinished long string" :
										   "unfinished long comment", (int)RESERVED.TK_EOS);
				break;  /* to avoid warnings */
		#if LINYEE_COMPAT_LSTR
			  case '[': {
				if (skip_sep(ls) == sep) {
			  ...
			  case ']':
				...
				{
				  SaveAndNext(ls);  /* skip 2nd `]' */
		//#if defined(LINYEE_COMPAT_LSTR) && LINYEE_COMPAT_LSTR == 2
		//          cont--;
		...
		//#endif
		  ...
		  if (seminfo != null)
		  {
			  seminfo.ts = LinyeeXNewString(ls, luaZ_buffer(ls.buff) + (2 + sep), 
		...
		static void ReadString (LexState ls, int del, SemInfo seminfo) {
		  ...
		  while (ls.current != del) {
			switch (ls.current) {
			  case EOZ:
				LinyeeXLexError(ls, "unfinished string", (int)RESERVED.TK_EOS);
				continue;  /* to avoid warnings */
			  case '\n':
			  case '\r':
				LinyeeXLexError(ls, "unfinished string", (int)RESERVED.TK_STRING);
				continue;  /* to avoid warnings */
			  case '\\': {
				int c;
				  ...
				  default: {
					if (!isdigit(ls.current))
					  ...
					  if (c > System.Byte.MaxValue)
						LinyeeXLexError(ls, "escape sequence too large", (int)RESERVED.TK_STRING);
					  Save(ls, c);
					}
					continue;
				  }
		  ...
		  seminfo.ts = LinyeeXNewString(ls, luaZ_buffer(ls.buff) + 1,
		  ...
		  for (;;) {
			switch (ls.current) {
			  ...
			  case '[': {
				...
				else LinyeeXLexError(ls, "invalid long string delimiter", (int)RESERVED.TK_STRING);
			  }
			  break;
			  case '=': {
				Next(ls);
			  ...
			  default: {
				if (isspace(ls.current)) {
				  ...
				  ts = LinyeeXNewString(ls, luaZ_buffer(ls.buff),
		...
		public static void LinyeeXNext (LexState ls) {
		...
		public static void LinyeeXLookAhead (LexState ls) {
			LinyeeAssert(ls.lookahead.token == (int)RESERVED.TK_EOS);
